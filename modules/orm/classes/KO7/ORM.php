...
class KO7_ORM extends Model implements Serializable
{
    ...
    public function set($column, $value)
    {
        ...
        if (array_key_exists($column, $this->_object)) {
            ...
            if (Arr::get($this->_has_many[$column], 'update', false)) {
                ...
                throw new KO7_Exception(
                    'The :property: property is a to many relation in the :class: class',
                    [':property:' => $column, ':class:' => get_class($this)]
                );
            }
        } else {
    ...
    protected function get_typed($column)
    {
        ...
        switch ($this->table_column_type($column)) {
            ...
            case 'string':
                return strval($this->__get($column));
        }
    ...
    {
        if ($this->_loaded) {
            throw new KO7_Exception('Method find() cannot be called on loaded objects');
        }
    ...
    {
        if ($this->_loaded) {
            throw new KO7_Exception('Method find_all() cannot be called on loaded objects');
        }
    ...
    public function count_all()
    {
        ...
        $records = $this->_db_builder->from([$this->_table_name, $this->_object_name])
            ->select(
                [
                    DB::expr(
                        'COUNT(' . $this->_db->quote_column($this->_object_name . '.' . $this->_primary_key) . ')'
                    ),
                    'records_found',
                ]
            )
