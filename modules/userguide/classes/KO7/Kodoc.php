...
class KO7_Kodoc
{
    ...
    public static function link_class_member($matches)
    {
        ...
        if (isset($matches[3])) {
            // If the first char is a $ it is a property, e.g. KO7::$base_url
            if ($matches[3][0] === '$') {
     ...
     * >  I personally don't like this as it was used on the index page.  Way too much stuff on one page.  It has
     * potential for a package index page though.
     * >  For example:  class_methods( KO7::list_files('classes/sprig') ) could make a nice index page for the sprig
     * package in the api browser
     * >     ~bluehawk
     */
    ...
    {
        $api_packages = KO7::$config->load('userguide.api_packages');
    ...
    public static function is_transparent($class, $classes = null)
    {
        ...
        if (! $transparent_prefixes) {
            $transparent_prefixes = KO7::$config->load('userguide.transparent_prefixes');
        }
