...
class KO7_Minion_CLI
{
    ...
    public static function password($text = '')
    {
        ...
        if (KO7::$is_windows) {
            ...
            $password = shell_exec(
                '/usr/bin/env bash -c \'read -s -p "' . escapeshellcmd($text) . '" var && echo $var\''
            );
        }
    ...
    public static function color($text, $foreground, $background = null)
    {
        ...
        if (! array_key_exists($foreground, Minion_CLI::$foreground_colors)) {
            throw new KO7_Exception('Invalid CLI foreground color: ' . $foreground);
        }
        
        if ($background !== null and ! array_key_exists($background, Minion_CLI::$background_colors)) {
            throw new KO7_Exception('Invalid CLI background color: ' . $background);
        }
