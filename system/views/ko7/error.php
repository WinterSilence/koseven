...
<div id="koseven_error">
        ...
        <ol class="trace">
            ...
            foreach (Debug::trace($trace) as $i => $step): ?>
                <li>
                    <p>
                            <span class="file">
                                ...
                                if ($step['file']): $source_id = $error_id . 'source' . $i; ?>
                                    <a href="#<?php
                                    echo $source_id ?>" onclick="return toggle('<?php
                                    echo $source_id ?>')">
                                        <?php
                                        echo Debug::path($step['file']) ?> [ <?php
                                        echo $step['line'] ?> ]
                                    </a>
                                <?php
                                else: ?>
                                    {<?php
                                    echo I18n::get('PHP internal call') ?>}
                                <?php
                                endif ?>
                            </span>
                        &raquo;
                        ...
                        if ($step['args']): $args_id = $error_id . 'args' . $i; ?>
                            <a href="#<?php
                            echo $args_id ?>" onclick="return toggle('<?php
                            echo $args_id ?>')">
                                <?php
                                echo I18n::get('arguments') ?>
                            </a>
                        <?php
                        endif ?>
                        )
                    ...
                    if (isset($source_id)): ?>
                        <pre id="<?php
                        echo $source_id ?>" class="source collapsed">
                                <code><?php
                                    echo $step['source'] ?></code>
                            </pre>
                    <?php
                    endif ?>
                </li>
    ...
    <h2>
        <a href="#<?php
        echo $env_id = $error_id . 'environment' ?>" onclick="return toggle('<?php
        echo $env_id ?>')">
            <?php
            echo I18n::get('Environment') ?>
        </a>
    </h2>
    <div id="<?php
    echo $env_id ?>" class="content collapsed">
        ...
        <h3>
            <a href="#<?php
            echo $env_id = $error_id . 'environment_included' ?>" onclick="return toggle('<?php
            echo $env_id ?>')"><?php
                echo I18n::get('Included files') ?></a>
            (<?php
            echo count($included) ?>)
        </h3>
        <div id="<?php
        ...
        <h3>
            <a href="#<?php
            echo $env_id = $error_id . 'environment_loaded' ?>" onclick="return toggle('<?php
            echo $env_id ?>')"><?php
                echo I18n::get('Loaded extensions') ?></a>
            (<?php
            echo count($included) ?>)
        </h3>
        <div id="<?php
        ...
        foreach (['_SESSION', '_GET', '_POST', '_FILES', '_COOKIE', '_SERVER'] as $var): ?>
            ...
            <h3>
                <a href="#<?php
                echo $env_id = $error_id . 'environment' . strtolower($var) ?>" onclick="return toggle('<?php
                echo $env_id ?>')">$<?php
                    echo $var ?></a>
            </h3>
            <div id="<?php
            echo $env_id ?>" class="collapsed">
                <table cellspacing="0">
                    ...
                    foreach ($GLOBALS[$var] as $key => $value): ?>
                        <tr>
                            <td><code><?php
                                    echo htmlspecialchars((string) $key, ENT_QUOTES, KO7::$charset, true); ?></code>
                            </td>
                            <td>
                                <pre><?php
                                    echo Debug::dump($value) ?></pre>
                            </td>
                        </tr>
                    <?php
                    endforeach ?>
