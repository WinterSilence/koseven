...
<div class="ko7">
        ...
        <table class="profiler">
            <tr class="group">
                <th class="name" rowspan="2"><?php
                    echo I18n::get(ucfirst($group)) ?></th>
                <td class="time" colspan="4"><?php
                    echo number_format($group_stats[$group]['total']['time'], 6) ?> <abbr title="seconds">s</abbr></td>
            </tr>
            <tr class="group">
            ...
            <tr class="headers">
                <th class="name"><?php
                    echo I18n::get('Benchmark') ?></th>
                <?php
                foreach ($group_cols as $key): ?>
                    <th class="<?php
                    echo $key ?>"><?php
                        echo I18n::get(ucfirst($key)) ?></th>
                <?php
                endforeach ?>
            </tr>
            <?php
                        ...
                        echo $key ?>">
                            <div>
                                ...
                                if ($key === 'total'): ?>
                                    ...
                                    echo $group_stats[$group]['max']['time'] ? max(
                                        0,
                                        100 - $stats[$key]['time'] / $group_stats[$group]['max']['time'] * 100
                                    ) : '0' ?>%"></div>
                                <?php
                                endif ?>
                            </div>
                ...
                <tr class="mark memory">
                    ...
                    foreach ($group_cols as $key): ?>
                        ...
                        echo $key ?>">
                            <div>
                                <div class="value"><?php
                                    echo number_format($stats[$key]['memory'] / 1024, 4) ?> <abbr
                                        title="kilobyte">kB</abbr></div>
                                <?php
                                if ($key === 'total'): ?>
                                    <div class="graph" style="left: <?php
                                    echo $group_stats[$group]['max']['memory'] ? max(
                                        0,
    ...
    <table class="profiler">
        ...
        <tr class="final mark time">
            <th class="name" rowspan="2" scope="rowgroup"><?php
                echo I18n::get('Application Execution') . ' (' . $stats['count'] . ')' ?></th>
            <?php
